...
function getDecimalPlaces(min, max) {
  ...
  const minDecimalPlaces = minStr.includes(".")
    ? minStr.split(".")[1].length
    : 0;
  const maxDecimalPlaces = maxStr.includes(".")
    ? maxStr.split(".")[1].length
    : 0;
...
function getRandomNumber(min, max, step) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const randomValue =
      Math.round(randInt * step * Math.pow(10, decimalPlaces)) /
...
document.querySelector("form").addEventListener("submit", function (event) {
  ...
  if (!validateInput(min, max)) {
    document.getElementById("result").innerHTML =
      "<p style='color: red;'>請輸入有效的數字，且最小值應小於最大值。</p>";
    return;
  }
