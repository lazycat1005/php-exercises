...
function adjustForDecimalPlaces(str1, str2, decimalPlaces1, decimalPlaces2) {
  ...
  // 移除小數點
  let num1Str = str1.replace(".", "");
  let num2Str = str2.replace(".", "");

  // 補零到相同的小數位數
  num1Str = num1Str.padEnd(
    num1Str.length + (maxDecimalPlaces - decimalPlaces1),
    "0"
  );
  num2Str = num2Str.padEnd(
    num2Str.length + (maxDecimalPlaces - decimalPlaces2),
    "0"
  );

  const adjusted1 = parseInt(num1Str);
  const adjusted2 = parseInt(num2Str);
...
function compareWithToFixed(
  ...
  const toFixedResult = (
    parseFloat(originalStr1) + parseFloat(originalStr2)
