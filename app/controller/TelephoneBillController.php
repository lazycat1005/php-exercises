...
use App\Validator\TelephoneBillValidator;
...
class TelephoneBillController
{
    ...
    public function calculateBill($callDuration)
    {
        ...
        $html = "<p>通話時長: {$callDuration} 分鐘</p>";
        $html .= "<p>計算過程:</p>";
        $html .= "<ul>";
        if ($callDuration <= 600) {
            $html .= "<li>前 600 分鐘: " . number_format($callDuration * 0.5, 2) . " 元</li>";
        } else {
            $html .= "<li>前 600 分鐘: " . number_format(600 * 0.5, 2) . " 元</li>";
        }
        if ($callDuration > 600 && $callDuration <= 1200) {
            $html .= "<li>超過 600 未滿 1200 分鐘: " . number_format(($callDuration - 600) * 0.5 * 0.9, 2) . " 元</li>";
        } elseif ($callDuration > 1200) {
            $html .= "<li>超過 600 未滿 1200 分鐘: " . number_format(600 * 0.5 * 0.9, 2) . " 元</li>";
        }
        if ($callDuration > 1200) {
            $html .= "<li>超過 1200 分鐘以上: " . number_format(($callDuration - 1200) * 0.5 * 0.79, 2) . " 元</li>";
        }
        $html .= "</ul>";
        $html .= "<p>總金額為 NT$ " . number_format($billAmount, 2) . "</p>";
        $html .= "<h3>應繳金額為 NT$ " . round($billAmount) . "</h3>";
        return [
            'success' => true,
            'html' => $html
        ];
